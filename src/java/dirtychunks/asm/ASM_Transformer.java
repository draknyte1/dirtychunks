...
public class ASM_Transformer implements IClassTransformer{

	@Override
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		//Obfuscated
		if (name.equals("abq")) {
			System.out.println("********* INSIDE OBFUSCATED EXPLOSION TRANSFORMER ABOUT TO PATCH: " + name);
			return patchClassASM(name, basicClass, true);
		}
		//Dev
		if (name.equals("net.minecraft.world.Explosion")) {
			System.out.println("********* INSIDE EXPLOSION TRANSFORMER ABOUT TO PATCH: " + name);
	...
	//Add some Logging to chunk Loading
		...
		while(methods.hasNext())
		{
			...
			if ((m.name.equals(targetMethodName) && m.desc.equals("(Z)V")))
			{
				...
				while (iter.hasNext())
				{
					...
					if (currentNode.getOpcode() == FDIV) //What does this do? TODO
					{
